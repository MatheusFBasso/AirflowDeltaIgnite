...
class Gold(Now):
    ...
    def gold_bike_status(self):
        # --------------------------------------------------------------------------------------------------------------
        if not DeltaTable.isDeltaTable(self.spark, f'{spark_path}/data/warehouse/gold.db/divvy_bikes_status'):
            ...
            brew_silver_df: SparkDataFrame = self.spark.createDataFrame([], schema)
            brew_silver_df.write.format('delta') \
                .mode('overwrite') \
                .option("overwriteSchema", "True") \
                .save(f'{spark_path}/data/warehouse/gold.db/divvy_bikes_status')
        ...
        df:SparkDataFrame = (
            self.spark.read.format('delta').load(f'{spark_path}/data/warehouse/silver.db/divvy_bikes_status')
        ...
        delta_table_silver = DeltaTable.forPath(sparkSession=self.spark,
                                                path=f'{spark_path}/data/warehouse/gold.db/divvy_bikes_status')
    ...
    def gold_station_information(self):
        # --------------------------------------------------------------------------------------------------------------
        if not DeltaTable.isDeltaTable(self.spark, f'{spark_path}/data/warehouse/gold.db/divvy_station_information'):
            ...
            brew_silver_df.write.format('delta') \
                .mode('overwrite') \
                .option("overwriteSchema", "True") \
                .save(f'{spark_path}/data/warehouse/gold.db/divvy_station_information')
        ...
        self.log_message(show=self._SHOW_LOG, message='READING SILVER DATA | silver.divvy_station_information',
                         start=True)
        df:SparkDataFrame = (
            self.spark.read.format('delta').load(f'{spark_path}/data/warehouse/silver.db/divvy_station_information')
                .withColumn('last_updated_ts', col('last_updated').cast('TIMESTAMP')).alias('as_is')
                .join(
                    self.spark.read.format('delta').load(f'{spark_path}/data/warehouse/silver.db/divvy_station_information')
                        .withColumn('last_updated_ts', col('last_updated').cast('TIMESTAMP'))
        ...
        delta_table_silver = DeltaTable.forPath(sparkSession=self.spark,
                                                path=f'{spark_path}/data/warehouse/gold.db/divvy_station_information')
    ...
    def gold_station_status(self):
        # --------------------------------------------------------------------------------------------------------------
        if not DeltaTable.isDeltaTable(self.spark, f'{spark_path}/data/warehouse/gold.db/divvy_station_status'):
            ...
            brew_silver_df.write.format('delta') \
                .mode('overwrite') \
                .option("overwriteSchema", "True") \
                .save(f'{spark_path}/data/warehouse/gold.db/divvy_station_status')
        ...
        df:SparkDataFrame = (
            self.spark.read.format('delta').load(f'{spark_path}/data/warehouse/silver.db/divvy_station_status')
                .withColumn('last_updated_ts', col('last_updated').cast('TIMESTAMP')).alias('as_is')
                .join(
                    self.spark.read.format('delta').load(f'{spark_path}/data/warehouse/silver.db/divvy_station_status')
                        .withColumn('last_updated_ts', col('last_updated').cast('TIMESTAMP'))
        ...
        delta_table_silver = DeltaTable.forPath(sparkSession=self.spark,
                                                path=f'{spark_path}/data/warehouse/gold.db/divvy_station_status')
    ...
    def gold_system_pricing_plan(self):
        # --------------------------------------------------------------------------------------------------------------
        if not DeltaTable.isDeltaTable(self.spark, f'{spark_path}/data/warehouse/gold.db/divvy_system_pricing_plan'):
            ...
            brew_silver_df.write.format('delta') \
                .mode('overwrite') \
                .option("overwriteSchema", "True") \
                .save(f'{spark_path}/data/warehouse/gold.db/divvy_system_pricing_plan')
        ...
        self.log_message(show=self._SHOW_LOG, message='READING SILVER DATA | silver.divvy_system_pricing_plan',
                         start=True)
        df:SparkDataFrame = (
            self.spark.read.format('delta').load(f'{spark_path}/data/warehouse/silver.db/divvy_system_pricing_plan')
                .withColumn('last_updated_ts', col('last_updated').cast('TIMESTAMP')).alias('as_is')
                .join(
                    self.spark.read.format('delta').load(f'{spark_path}/data/warehouse/silver.db/divvy_system_pricing_plan')
                        .withColumn('last_updated_ts', col('last_updated').cast('TIMESTAMP'))
        ...
        delta_table_silver = DeltaTable.forPath(sparkSession=self.spark,
                                                path=f'{spark_path}/data/warehouse/gold.db/divvy_system_pricing_plan')
    ...
    def gold_vehicle_types(self):
        # --------------------------------------------------------------------------------------------------------------
        if not DeltaTable.isDeltaTable(self.spark, f'{spark_path}/data/warehouse/gold.db/divvy_vehicle_types'):
            ...
            brew_silver_df.write.format('delta') \
                .mode('overwrite') \
                .option("overwriteSchema", "True") \
                .save(f'{spark_path}/data/warehouse/gold.db/divvy_vehicle_types')
        ...
        df: SparkDataFrame = (
                self.spark.read.format('delta').load(f'{spark_path}/data/warehouse/silver.db/divvy_vehicle_types')
                    .withColumn('last_updated_ts', col('last_updated').cast('TIMESTAMP')).alias('as_is')
                    .join(
                        self.spark.read.format('delta').load(f'{spark_path}/data/warehouse/silver.db/divvy_vehicle_types')
                            .withColumn('last_updated_ts', col('last_updated').cast('TIMESTAMP'))
        ...
        delta_table_silver = DeltaTable.forPath(sparkSession=self.spark,
                                                path=f'{spark_path}/data/warehouse/gold.db/divvy_vehicle_types')
