...
class BreweryAPI(Now):
    ...
    def _save_file(self, save_data: list, file_name: str = 'extracted_at_') -> None:
        ...
        if not os.path.exists(f"""{raw_bronze}/"""):
            os.makedirs(f"""{raw_bronze}/""")

        with open(f"""{raw_bronze}/{file_name}.json""", 'w') as output:
    ...
    def _files_validation(self, expected_total_responses: int) -> None:
        ...
        _files = sorted([x for x  in list(glob(f'''{raw_bronze}/*''')) if x.endswith('.json')])
