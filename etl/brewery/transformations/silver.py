...
class Silver(Now):
    ...
    def execute(self):
        ...
        df = self.spark.read.schema(self._SCHEMA_RAW_DATA).json(bronze_path_raw_data, multiLine=True)
        ...
        if df.count() > 0:
            ...
            df.write\
              ...
              .option('silver.brewery_daily', 'data/warehouse/')\
              .partitionBy('date_ref_carga')\
              .save('data/warehouse/silver.db/brewery_daily')
            self.log_message(show=self._SHOW_LOG, message='SAVING DAILY TABLE | OK', end=True)
        # --------------------------------------------------------------------------------------------------------------
            if not DeltaTable.isDeltaTable(self.spark, 'data/warehouse/silver.db/brewery'):
                self.log_message(show=self._SHOW_LOG, message='SAVING BREWERY TABLE | FIRST EXECUTION', start=True)
                df.write \
                    .format('delta') \
                    .mode('append') \
                    .option("overwriteSchema", "True") \
                    .option('silver.brewery', 'data/warehouse/') \
                    .partitionBy('date_ref_carga') \
                    .save('data/warehouse/silver.db/brewery')
                ...
                deltaTableSilver = DeltaTable.forPath(sparkSession=self.spark, path='data/warehouse/silver.db/brewery')
